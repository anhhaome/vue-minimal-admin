...
export default {
  install: (app, options) => {
    app.provide("mdb", {
      ...
      async init(name, version, tables) {
      ...
      async list(storeName, query = {}) {
        const { doc, controls } = extractControls(query, [
          "$limit",
          "$skip",
          "$sort",
          "$search",
        ...
        if (controls.$search && controls.$search.$text) {
          queryBuilder = queryBuilder
            .where("title")
            .startsWithIgnoreCase(controls.$search.$text);
        }

        if (Object.keys(doc).length > 0) {
          queryBuilder = queryBuilder.filter((originDoc) => {
            for (let key in doc) if (originDoc[key] !== doc[key]) return false;
