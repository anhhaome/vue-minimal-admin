...
// prepare input
const BLACK_LIST = ['logo-medium', 'color-wand']

const data = JSON.parse(readFileSync(join(input, './src/data.json')))
  .icons.reduce((result, item) => {
    if (!/(-(outline|sharp)){1}$/.test(item.name)) result.push(item)
...
const loadSvgContent = (iconName, iconType) => {
  ...
  if (!existsSync(filePath)) filePath = join(input, './src/svg', `${iconName}.svg`)
