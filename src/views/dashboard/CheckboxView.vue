...
<template>
  ...
  <MPanel>
    ...
    <MCheckbox :modelValue="true" variant="danger" label="Click to toggle check" />
    <MCheckbox variant="info" :indeterminate="true" label="Click to toggle check" />
    <MCheckbox :modelValue="true" variant="warn" label="Click to toggle check" />
  </MPanel>

  <MPanel>
    <p class="mb-2">Value: <code>{{ v }}</code></p>
    <MCheckbox v-model="v" variant="primary" label="Click to toggle check" />
  </MPanel>
  <MPanel>
    <p class="mb-2">Value: <code>{{ checkedList }}</code></p>

    <ul>
      <li v-for="item in checkList">
        <MCheckbox @update:modelValue="toggleValue(item, $event)" :label="item"/>
      </li>
    </ul>
  </MPanel>
</template>
